...
| D9 | **Ретро-фотография** | Старые, повреждённые, чёрно-белые фотографии | "Старое семейное фото 1950-х", "Выцветший снимок с царапинами" |
| D10 | **Предметы искусства** | Статуи, скульптуры, картины как объекты | "Мраморная статуя в музее", "Картина в золотой раме" |
| D11 | **Мультипликация** | Рисунки, анимационный стиль, комиксы | "Персонаж в стиле Disney", "Аниме-портрет девушки" |
| D12 | **Текстуры** | Поверхности, материалы, паттерны | "Текстура дерева дуба", "Бесшовная текстура кирпичной стены" |
| D13 | **Сканы документов** | Отсканированные документы, бланки, формы | "Скан старинной рукописи", "Отсканированный паспорт" |
...
| T4 | **Style Transfer** | I2I | Перенос художественного стиля (рисунок→акварель, рисунок→фото и т.д.) |
...
| T9 | **Object Removal** | I2I | Удаление объекта с изображения |
| T10 | **Object Addition** | I2I | Добавление нового объекта на изображение |
| T11 | **Background Removal** | I2I | Удаление фона (выделение объекта на прозрачном фоне) |
| T12 | **Image Restoration** | I2I | Реставрация старых/повреждённых фотографий |
| T13 | **Colorization** | I2I | Раскрашивание чёрно-белых изображений |
| T14 | **Viewpoint Change** | I2I | Изменение ракурса/перспективы изображения |
| T15 | **Watermark Removal** | I2I | Удаление водяных знаков |
| T16 | **Image Compositing** | I2I | Совмещение частей разных изображений |
| T17 | **Object Modification** | I2I | Изменение атрибутов объектов (цвет, форма, дорисовка частей) |
| T18 | **Expression Change** | I2I | Изменение эмоций/выражений лиц |

*\* T8 — опциональная задача*

#### 3.1.3 Матрица применимости

**Часть 1: Базовые задачи (T1-T8)**
...
**Часть 2: Расширенные задачи (T9-T18)**
...
**Легенда задач:**
- T9: Object Removal (убери объект)
- T10: Object Addition (добавь объект)
- T11: Background Removal (убери фон)
- T12: Image Restoration (реставрация)
- T13: Colorization (раскрась)
- T14: Viewpoint Change (измени ракурс)
- T15: Watermark Removal (убери водяной знак)
- T16: Image Compositing (совмести части)
- T17: Object Modification (измени объекты)
- T18: Expression Change (измени эмоцию)

**Легенда символов:** ✓ — комбинация релевантна, - — комбинация не применима или редка, * — опциональная задача

### 3.2 Требования к размеру выборки

| Уровень | Сэмплов на ячейку | Всего (при ~70 ячейках)* | Назначение |
|---------|-------------------|--------------------------|------------|
| Минимальный | 50 | 3500 | Пилотное исследование |
| Рекомендуемый | 100 | 7000 | Базовая оценка |
| Оптимальный | 150 | 10500 | Статистически значимые результаты |

*\* Количество ячеек зависит от применимости комбинаций домен×задача (13 доменов × 8 задач, с учётом матрицы применимости)*
...
> **Код реализации:** см. [`src/validate_generation.py`](src/validate_generation.py)

**Чеклист автоматической валидации сгенерированного изображения:**

**Техническая корректность:**
- [ ] **Разрешение** — изображение имеет размер не менее 1024×1024 пикселей
- [ ] **Формат** — файл сохранён в формате PNG (без сжатия с потерями)
- [ ] **Не чёрное** — среднее значение пикселей > 10 (не полностью чёрное изображение)
- [ ] **Не белое** — среднее значение пикселей < 245 (не полностью белое изображение)
- [ ] **Не повреждено** — файл корректно читается и не содержит ошибок

**Базовое качество:**
- [ ] **Не размытое** — дисперсия Лапласиана > 100 (достаточная резкость)
- [ ] **Есть контент** — плотность границ > 0.1 (изображение содержит значимые объекты)

**Критерии отклонения:**
- Изображение отклоняется, если хотя бы одна проверка не пройдена
- Отклонённые изображения требуют повторной генерации с другими параметрами
...
dataset:
  ...
  tasks:
    ...
    - enhancement
    - controlled_generation  # optional
    # Расширенные задачи (T9-T18)
    - object_removal
    - object_addition
    - background_removal
    - image_restoration
...
|--------|------|-----------|
| 0.3 | 2026-02-02 | Первоначальная версия документа |
